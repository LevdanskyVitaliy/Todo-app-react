...
function App() {
  ...
  const toggleDone = async (id) => {
    ...
    setTodos(
      todos.map((todo) =>
        todo.id === id ? { ...todo, done: updatedDone } : todo
      )
    );
    ...
    return (
      <div className="text-white ">
        <p className="text-md font-semibold text-gray-500">
          Selected date:{" "}
          {selected ? selected.toLocaleDateString() : "All time tasks"}
        </p>
        <DayPicker
          timeZone="UTC"
          mode="single"
  ...
  const DateTasks = () => {
    return (
      <div className="mx-auto justify-center items-center md:m-1 m-5 ">
        <p className="mt-auto text-md text-white md:text-xl ">
          List contains -{" "}
          <span
            className={totalCount !== 0 ? "text-red-500" : "text-green-500"}
          >
            {totalCount}
          </span>{" "}
          items
  ...
  const handleDelete = async (id) => {
    ...
    const confirmed = window.confirm(
      "Are you sure you want to delete a To-do?"
    );
    if (confirmed) {
  ...
  const renderFilteredTodos = (filtered) => {
    if (!filtered.length)
      return <p className="text-white text-center">No todos found.</p>;
    return (
  ...
  const List = () => {
    ...
    const baseTodos = searchQuery ? searchResults : todos;
  
    const filteredTodos = showUndoneOnly
      ? baseTodos.filter(todo => !todo.done)
      : baseTodos;
      ...
      <div className="md:flex gap-20 py-10">
        <div className="md:w-3/8  ">
          <div className="md:full md:fixed flex-col  border-solid border-1 border-white rounded-xl px-8 md:pb-40 mx-7 ">
            <form
              className="my-3 flex flex-col justify-between gap-1"
              onSubmit={handleSubmit}
            >
              <input
                className="bg-white hover:bg-gray-100 p-5 mb-1 rounded-lg md:w-full w-2/3 mx-auto"
                type="text"
                name="todoName"
            ...
            <div
              ...
              style={{ width: "270px", height: "200px", margin: "0 auto" }}
            >
              <MyDatePicker />
            </div>
          </div>
